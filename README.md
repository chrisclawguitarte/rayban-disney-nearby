...
- Filters for both parks, Disneyland only, DCA only, open attractions, rides-only, low waits, headliners, every resort land, and quick ride focuses for Rise, Radiator Springs Racers, Guardians, WEB SLINGERS, Space/Hyperspace Mountain, and Indiana Jones.
- Single-rider wait rows are hidden from every filter so they do not appear as separate nearby attractions.
...
While the app is open, it polls the deployed snapshot every 5 minutes with cache-busting and no-cache request headers. It also refreshes when the WebView becomes visible, regains focus, restores from page cache, or comes back online after the current snapshot is stale. The status line shows both the latest Queue-Times ride timestamp and the age of the deployed app snapshot.
...
The scheduled refresh workflow runs every 5 minutes. Each run regenerates `waits.json`, validates the app bundle, and deploys the refreshed static site to GitHub Pages without committing a new wait snapshot every time. The in-browser app refetches `waits.json` every 5 minutes, so an already-open glasses session picks up each deployed snapshot on the same cadence without needing a manual reload.
